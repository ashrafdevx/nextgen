...
const OurBlogs = () => {
  ...
  let currentBlogs;
  if (pathname === "/") {
    // Show only 3 blogs if the path is "/"
    currentBlogs = blogData.slice(0, 3);
  } else {
    const indexOfLastBlog = currentPage * blogsPerPage;
    const indexOfFirstBlog = indexOfLastBlog - blogsPerPage;
    currentBlogs = blogData.slice(indexOfFirstBlog, indexOfLastBlog);
  }
    ...
    <>
      <div className="container mx-auto px-2 sm:py-16 sm:px-2 md:py-16 lg:py-2">
        <div className="container flex-col items-center flex pb-6  ">
          {pathname == "/" ? (
            <h1 className="text-3xl w-full px-8 sm:text-4xl md:text-5xl font-medium text-gray-800">
              Our Blogs
            </h1>
          ) : (
